...
if __name__ == "__main__":
    ...
    for num_itr in range(3):
        for prune_percentage in [0.9, 0.8, 0.5]:
            print("Now processing CIFAR10...")
            root_dir = "./data"
            # Using CIFAR10 train and test sets as an example
            trainset = CIFAR10(root=root_dir, train=True, download=True, transform=transform)
            ...
            fid_value = calculate_fid_given_datasets(
                dataset1=trainset,
                dataset2=pruned_trainset,
                batch_size=50,
                device=device,
                dims=dims,
                num_workers=num_workers
            )
            print(f"FID between Full CIFAR10 Trainset and {int((1 - prune_percentage) * 100)}% Random CIFAR10 trainset: {fid_value:.4f}")

            # now do stratified sampling
            label_to_indices = defaultdict(list)
            for idx in range(len(trainset)):
                _, label, sample_idx = trainset[idx]  # trainset returns (img, label, sample_idx)
                label_to_indices[label].append(sample_idx)

            # Perform stratified pruning
            indices_to_keep = []

            # for each label keep (1 - prune_percentage) of the samples
            for label, indices in label_to_indices.items():
                num_samples_to_keep = int((1 - prune_percentage) * len(indices))
                keep_indices = random.sample(indices, num_samples_to_keep)
            ...
            print(f"FID between Full CIFAR10 Trainset and {int((1 - prune_percentage) * 100)}%  Stratified Pruned CIFAR10 trainset: {fid_value:.4f}")


            # subset based on scores
            json_path = "/nfs/homedirs/dhp/unsupervised-data-pruning/scores/prune/cifar10_zcoreset_0_5_26.json"
            ...
            print(f"FID between Full CIFAR10 Trainset and Zcoreset Top {int((1 - prune_percentage) * 100)}% CIFAR10 trainset: {fid_value:.4f}")