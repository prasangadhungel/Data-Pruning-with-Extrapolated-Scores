...
def main(cfg_path: str):
    ...
    ratio=cfg.pruning.percentages
    #1.Substitute dataset with InfoBatch dataset, optionally set r; delta and num_epoch are needed to anneal with full data
    trainset = InfoBatch(trainset, ratio if ratio else None, cfg.pruning.num_epochs, args.delta)
    
    trainloader = torch.utils.data.DataLoader(
    trainset, batch_size=cfg.training.batch_size, shuffle=True, sampler = trainset.pruning_sampler())
    ...
    for num_itr in range(cfg.experiment.num_iterations):
        ...
        model_name = f"{cfg.paths.models}/infobatch_last"
        if cfg.dataset.for_extrapolation.value is True:
        ...
        if cfg.pruning.prune is True:
            prune(
                ...
                cfg=cfg,
                wandb_name="infobatch-last-scheduler-",
                device=device,
            )
...
if __name__ == "__main__":
    default_config_path = os.path.join(
        os.path.dirname(__file__), "configs", "infobatch_config.yaml"
    )
    config_path = parse_config(
        default_config=default_config_path, description="Run infobatch Pruning"
    )
