...
def main(cfg_path: str):
    ...
    for model_name in tqdm(cfg.models.names):
        if cfg.checkpoints.read_embeddings:
            ...
            num_params_embedding = count_parameters(embedding_model)
            logger.info(f"Embedding Model Params ({model_name}): {num_params_embedding:,}")
            # wandb.log({"Embedding Model Params": num_params_embedding})

            sample_input, sample_target, sample_idx = trainset[0]  # first sample, ignore label & idx
            ...
            for images, labels, sample_idxs in tqdm(
                ...
                with torch.no_grad():
                    ...
                    probs = torch.softmax(logits, dim=1) # shape (B, num_classes)

                    # probability at the true label
                    batch_pred_mean = probs.gather(1, labels.unsqueeze(1)).squeeze(1)  # shape (B,)
        ...
        for k in tqdm(cfg.hyperparams.k_values):
            ...
            num_params_gnn = count_parameters(model)
            logger.info(f"GNN Params (k={k}): {num_params_gnn:,}")
            wandb.log({"GNN Params": num_params_gnn}, step=0)
            ...
            # find samples with highest extrapolated score
            topk=10
            topk_indices = np.argpartition(extrapolated_scores, -topk)[-topk:]  # unordered
            topk_pred_mean = pred_mean[topk_indices].cpu().numpy()
            mu_d = topk_pred_mean.mean()
            ...
            prune(
                ...
                scores_dict=saved_scores,
                pred_mean=pred_mean.tolist(),
                mu_d=mu_d,
                cfg=cfg.prune,
                wandb_name="gnn-du-beta",
