...
def run(
    ...
    start = time.time()
    (adj_matrix, attr_matrix, labels, train_idx, val_idx, test_idx) = utils.get_data(
    ...
    results = {
        "accuracy_train": 100
        * accuracy_score(labels[train_idx], predictions[train_idx]),
        ...
        "f1_train": f1_score(
            labels[train_idx], predictions[train_idx], average="macro"
        ),
