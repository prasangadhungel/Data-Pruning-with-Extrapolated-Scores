...
def get_correlation(
    ...
    # Convert to numpy for cuML (ensure data is on GPU)
    seed_np = seed_embeddings.detach().cpu().numpy()
    unseed_np = unseed_embeddings.detach().cpu().numpy()

    seed_cudf = cudf.DataFrame(seed_np)
    unseed_cudf = cudf.DataFrame(unseed_np)

    # Use cuML KNN
    metric = "euclidean" if distance_metric != "cosine" else "cosine"
    model = NearestNeighbors(n_neighbors=k, metric=metric)
    model.fit(seed_cudf)

    distances, indices = model.kneighbors(unseed_cudf)  # indices: shape [U, k]

    # Get scores of the k neighbors for each unseeded point
    seed_scores_tensor = torch.tensor(
    ...
    neighbor_indices = torch.tensor(indices.to_numpy(), dtype=torch.long, device=device)
    neighbor_scores = seed_scores_tensor[neighbor_indices]  # shape [U, k]

    # Compute average and weighted scores
    knn_avg_scores = neighbor_scores.mean(dim=1)  # shape [U]

    distances_tensor = torch.tensor(
        distances.to_numpy(), dtype=torch.float, device=device
    )
    weights = torch.exp(-distances_tensor)
    knn_weighted_scores = (neighbor_scores * weights).sum(dim=1) / weights.sum(dim=1)
